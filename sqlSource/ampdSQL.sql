...
NOT EXISTS `mathLearningDB`.`SURVEY_RESPONSE` (
  ...
  `ClassID` INT NOT NULL,
  `SurveyQID` INT NOT NULL,
  `SurveyAnswer` FLOAT NULL,
  PRIMARY KEY (`UserID`, `Timestamp`, `ClassID`, `SurveyQID`),
  -- INDEX `CID_idx` (`ClassID` ASC) VISIBLE,
  -- INDEX `SID_idx` (`SurveyQID` ASC) VISIBLE,
  CONSTRAINT `SURVEYR_CID`
    FOREIGN KEY (`ClassID`)
    REFERENCES `mathLearningDB`.`CLASS` (`ClassID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
--  CONSTRAINT `SURVEYR_SID`
--    FOREIGN KEY (`SurveyQID`)
--    REFERENCES `mathLearningDB`.`SURVEY_QUESTION` (`SurveyQID`)
--    ON DELETE NO ACTION
--    ON UPDATE NO ACTION,
  CONSTRAINT `SURVEYR_UID`
    FOREIGN KEY (`UserID`)
